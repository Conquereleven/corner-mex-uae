...
assert.doesNotMatch(
  sql,
  /\b(stock|quantity_on_hand)\b[^;\n]*(default|values?)\s*\(?\s*(50|100)\b/i,
  "stock fixtures are forbidden",
);
assert.doesNotMatch(
  sql,
  /create\s+table\s+(?:public\.)?(sellers|seller_payouts|commissions|storefronts)\b/i,
  "marketplace entities are forbidden",
);
assert.doesNotMatch(
  sql,
  /grant\s+all\s+on[^;]+to\s+(public|anon)/i,
  "broad public grants are forbidden",
);
...
for (const table of [
  ...
  assert.match(
    sql,
    new RegExp(`alter\\s+table\\s+public\\.%I enable row level security`, "i"),
    "RLS loop missing",
  );
}
...
console.log(
  JSON.stringify({
    status: "approved_for_target_application",
    checksum: actual,
    seeds: 0,
