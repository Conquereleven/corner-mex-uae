...
export function validateProgramState() {
  ...
  assert(current.authority.repository === "Conquereleven/corner-mex-uae", "PROGRAM_REPOSITORY_INVALID");
  ...
  assert(current.authority.expectedMainSha === current.authority.observedMainSha, "PROGRAM_MAIN_DRIFT");
  ...
  assert(current.platforms.supabase.projectRef === "wlrfknmrhowldygmvtvn", "PROGRAM_SUPABASE_IDENTITY_INVALID");
  ...
  assert(current.safety.writesBlocked && current.safety.externalSendsBlocked && current.safety.customerImpactBlocked, "PROGRAM_SAFETY_GATE_OPEN");

  assert(registry.schemaVersion === "cornermex-deployment-registry-v1", "DEPLOYMENT_REGISTRY_VERSION_INVALID");
  assert(registry.expectedCommit === current.authority.observedMainSha, "DEPLOYMENT_EXPECTED_SHA_MISMATCH");
  ...
  for (const deployment of registry.deployments) {
    ...
    assert(deployment.failureClassification === "package_manager_lockfile_mismatch", "DEPLOYMENT_ROOT_CAUSE_INVALID");
  ...
  assert(handoff.$schema === "https://json-schema.org/draft/2020-12/schema", "HANDOFF_SCHEMA_DRAFT_INVALID");
  assert(handoff.properties.repository.const === "Conquereleven/corner-mex-uae", "HANDOFF_REPOSITORY_INVALID");
