...
requireMatch(
  redirect,
  /value\.startsWith\("\/\/"\)/,
  "protocol-relative redirects must fail closed",
);
...
requireMatch(
  ...
  "Admin link must be role-conditional",
);
requireMatch(account, /supabase\.auth\.signOut\(\)/, "sign-out must remain available");
rejectMatch(
  account,
  /AdminBootstrapCard|adminBootstrap|Claim admin/i,
  "admin bootstrap UI is forbidden",
);
...
rejectMatch(
  ...
  "unsupported stock or delivery claims are forbidden",
);
...
rejectMatch(
  checkout,
  /quote-selection|useQuoteSelection|cm\.quoteSelection/,
  "checkout must not read B2B state",
);
...
requireMatch(
  orders,
  /\.handler\(async \(\{ data, context \}\) => \{\s*assertCheckoutExecutionEnabled\(\);/,
  "placeOrder gate must be the first handler statement",
);
...
if (paymentGates < 3)
  failures.push(`${payments}: every reachable payment/confirmation function must fail closed`);

const quote = "src/routes/b2b_.quote.tsx";
rejectMatch(
  quote,
  /placeOrder|createStripeSession|useCart|cornermex-cart-v1/,
  "B2B quote must not enter B2C execution",
);

const boundaries = "src/lib/commerce-flow-boundaries.ts";
requireMatch(
  boundaries,
  /storage: "localStorage"[\s\S]*storage: "sessionStorage"/,
  "storage types must remain independent",
);
...
if (failures.length) {
  console.error(
    "CM-COM-1C-R1 validation failed:\n" + failures.map((failure) => `- ${failure}`).join("\n"),
  );
  process.exit(1);
}

console.log(
  "CM-COM-1C-R1 validation passed: auth/admin, B2C fail-closed checkout, and B2B isolation are intact.",
);
