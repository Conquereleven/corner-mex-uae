...
for (const file of files) {
  ...
  if (/sb_secret_[A-Za-z0-9_-]{12,}/.test(source)) errors.push(`Supabase secret key in browser bundle: ${file}`);
  for (const match of source.matchAll(/eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}/g)) {
    try {
...
console.log(`browser secret scan valid: files=${files.length}, publicAnonJwt=${publicAnonJwtCount}, serviceRoleSecrets=0`);
