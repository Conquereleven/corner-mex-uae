...
function AdminProducts() {
  ...
  const products = useMemo(() => {
    ...
    return (query.data ?? []).filter((product) => {
      const name = product.product_translations?.find((item) => item.lang === "en")?.name ?? "";
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Canonical single-merchant CornerMex catalog management.
          </p>
        </div>
        <Button asChild>
            ...
            <Table>
              ...
              <TableBody>
                {products.map((product) => {
                  const name =
                    product.product_translations?.find((item) => item.lang === "en")?.name ??
                    product.slug;
                  const variants = product.product_variants ?? [];
                  const active = variants.filter((item) => item.is_active);
                  const stock = active.reduce(
                    (sum: number, item) => sum + Number(item.stock ?? 0),
                    0,
                  );
                  const prices = active
                    .map((item) => Number(item.price_aed))
