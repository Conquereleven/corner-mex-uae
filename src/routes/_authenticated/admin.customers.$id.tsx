  ...
  if (q.isError || !q.data)
    return (
      <EmptyState
        ...
        description={(q.error as Error)?.message ?? "Customer not found"}
        action={
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi
          label={t("dash.customers.detail.lastOrder")}
          value={stats.last_order ? new Date(stats.last_order).toLocaleDateString() : "—"}
        />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("dash.customers.detail.profile")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            ...
            <Row
              label={t("dash.customers.detail.lang")}
              value={(profile.preferred_lang ?? "").toUpperCase()}
            />
            <Row
              label={t("dash.customers.detail.joined")}
              value={new Date(profile.created_at).toLocaleDateString()}
            />
            <Row
              label={t("dash.customers.detail.firstOrder")}
              value={stats.first_order ? new Date(stats.first_order).toLocaleDateString() : "—"}
            />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              ...
              {t("dash.customers.detail.addresses")}
            </CardTitle>
          </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            {t("dash.customers.detail.orders")}
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {orders.length === 0 ? (
            <EmptyState
              icon={ShoppingCart}
              title={t("dash.customers.detail.noOrders")}
                  ...
                  <TableRow key={o.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className="capitalize"
                        style={{ borderColor: statusColor(o.status), color: statusColor(o.status) }}
                      >
                        {o.status}
                      </Badge>
                    </TableCell>
