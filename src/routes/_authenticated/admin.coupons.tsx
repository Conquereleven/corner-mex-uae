...
function CouponsAdmin() {
  ...
  const q = useQuery({ queryKey: ["admin-coupons"], queryFn: () => list({ data: { scope: "all" } }) });
  ...
  const m = useMutation({
    ...
    onSuccess: () => { toast.success("Coupon saved"); qc.invalidateQueries({ queryKey: ["admin-coupons"] }); },
      ...
      <Card>
        ...
        <CardContent>
          {q.isLoading ? <p className="text-sm text-muted-foreground">Loading…</p> :
           (q.data ?? []).length === 0 ? <p className="text-sm text-muted-foreground">No coupons yet.</p> : (
