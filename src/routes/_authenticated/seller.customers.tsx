...
function SellerCustomers() {
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((c) => [c.full_name, c.email, c.phone, c.company_name].some((x: any) => (x ?? "").toLowerCase().includes(s)));
  }, [all, search]);
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {q.isLoading ? (
            ...
            <EmptyState icon={Users} title="No customers yet" description="As buyers place orders, they'll appear here." />
          ) : (
                ...
                <TableBody>
                  {rows.map((c: any) => (
                    <TableRow key={c.id} className="hover:bg-muted/40">
                      <TableCell>
                        <Link to="/seller/customers/$id" params={{ id: c.id }} className="font-medium hover:underline">
                          {c.full_name}
                        </Link>
                      ...
                      <TableCell className="text-right font-mono tabular-nums">{AED(c.gmv)}</TableCell>
                      <TableCell className="text-right text-xs text-muted-foreground">
                        {c.last_order_at ? new Date(c.last_order_at).toLocaleDateString() : "—"}
                      </TableCell>
                      <TableCell className="text-right">
                        <Link to="/seller/customers/$id" params={{ id: c.id }} className="text-muted-foreground hover:text-foreground">
