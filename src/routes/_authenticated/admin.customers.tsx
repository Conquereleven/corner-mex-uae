...
function AdminCustomers() {
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((c) =>
      [c.full_name, c.email, c.phone, c.company_name].some((x) => (x ?? "").toLowerCase().includes(s)),
    );
  }, [all, search]);
      ...
      <Card>
        <CardHeader>
          <div className="relative">
            ...
            <Input placeholder={t("dash.customers.searchPh")} value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
          </div>
        </CardHeader>
                ...
                <TableBody>
                  {rows.map((c) => (
                    <TableRow key={c.id} className="cursor-pointer hover:bg-muted/40">
                      ...
                      <TableCell><Badge variant="outline" className="uppercase text-[10px]">{c.preferred_lang}</Badge></TableCell>
                      <TableCell className="text-right">
                        <Link to="/admin/customers/$id" params={{ id: c.id }} className="text-muted-foreground hover:text-foreground">
