      ...
      <SiteLayout>
        <section className="mx-auto max-w-3xl px-4 py-24 text-center sm:px-6 lg:px-8">
          ...
          <p className="mt-4 text-muted-foreground">
            Start browsing the catalogue to add Mexican goods.
          </p>
          <Link to="/shop">
            <Button className="mt-8 rounded-full bg-foreground text-background hover:bg-foreground/90">
              Go to shop
            </Button>
          </Link>
        </section>
    ...
    <SiteLayout>
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Sold directly by CornerMex · {items.length} item{items.length > 1 ? "s" : ""}
        </p>
                        ...
                        <div className="mt-auto flex items-center justify-between pt-3">
                          ...
                          <div className="flex items-center gap-4">
                            <span className="text-sm font-medium">
                              AED {(it.unitPrice * it.qty).toFixed(2)}
                            </span>
                            <button
          ...
          <DesertGlassDrawerShell className="h-fit rounded-3xl p-6">
            ...
            <dl className="mt-6 space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd>{totals.shipping === 0 ? "Free" : `AED ${totals.shipping.toFixed(2)}`}</dd>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between border-t border-border pt-3 text-base font-medium">
                <dt>Total</dt>
                <dd>AED {totals.total.toFixed(2)}</dd>
              </div>
            </dl>
            {totals.subtotal < 150 && (
            ...
            {checkoutEnabled ? (
              ...
              <Button size="lg" disabled className="mt-6 w-full rounded-full">
                Checkout coming soon
              </Button>
            )}
            <p className="mt-3 text-center text-[11px] text-muted-foreground">
              Checkout and payment remain off until an authorized launch.
            </p>
