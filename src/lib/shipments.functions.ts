...
import { sendExternalEmail } from "@/lib/external-email.server";
...
// Outbound email is delegated to the canonical gate in external-email.server.ts,
// which requires CORNERMEX_EXTERNAL_EMAIL_ENABLED === "true" before provider
// configuration is even consulted. Provider keys alone never authorize a send.
async function sendEmail(to: string, subject: string, html: string) {
  return sendExternalEmail({ to, subject, html, logPrefix: "[email]" });
}

async function loadOrderEmailCtx(
  orderId: string,
): Promise<{ ctx: OrderEmailContext; buyerEmail: string | null } | null> {
  ...
  // No custom domain is owned or operational (FD-CM-PUBLIC-CONTACT-001), so the
  // verified application origin is the only fallback.
  const publicOrigin = process.env.PUBLIC_SITE_URL || siteOrigin();
  return {
...
export const sendOrderEmail = createServerFn({ method: "POST" })
  .inputValidator(
    (i: { orderId: string; kind: "order_placed" | "shipped" | "delivered"; shipmentId?: string }) =>
      z
        .object({
  ...
  .handler(async ({ data }) => {
    ...
    await logNotification(data.orderId, data.kind, r.ok ? "sent" : "failed", {
      to: loaded.buyerEmail,
...
export const sellerListShipments = createServerFn({ method: "GET" })
  ...
  .inputValidator((i: { orderId?: string }) =>
    z.object({ orderId: z.string().uuid().optional() }).parse(i),
  )
...
export const sellerCreateShipment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (
      (allItems ?? []).every(
        (x: any) => x.fulfillment_status === "shipped" || x.fulfillment_status === "delivered",
      )
    ) {
      await supabaseAdmin
        .from("orders")
    ...
    try {
      ...
      if (loaded?.buyerEmail) {
        ...
        await logNotification(data.orderId, "shipped", r.ok ? "sent" : "failed", {
    ...
    try {
      ...
      if (ord) {
        await createNotification({
          ...
          body: data.trackingNumber
            ...
            : `${data.carrier.toUpperCase()} shipment created.`,
          link: "/account",
      ...
      console.error("notify shipped failed", e);
    }
...
export const sellerMarkDelivered = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: { shipmentId: string }) =>
    z.object({ shipmentId: z.string().uuid() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    const { data: seller } = await supabaseAdmin
      .from("sellers")
    ...
    const { data: sh, error } = await supabaseAdmin
      .from("shipments")
    ...
    if ((allItems ?? []).every((x: any) => x.fulfillment_status === "delivered")) {
      ...
      try {
        ...
        if (loaded?.buyerEmail) {
          ...
          const r = await sendEmail(loaded.buyerEmail, tpl.subject, tpl.html);
          await logNotification(sh.order_id, "delivered", r.ok ? "sent" : "failed", {
            to: loaded.buyerEmail,
          });
        }
      } catch (e) {
      ...
      try {
        ...
        console.error("notify delivered failed", e);
      }
    }
    ...
    try {
      ...
      console.error("notify seller delivered failed", e);
    }
