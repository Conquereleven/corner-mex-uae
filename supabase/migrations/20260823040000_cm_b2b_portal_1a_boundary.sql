-- CM-B2B-PORTAL-1A
-- Account-scoped portal boundary over the private B2B foundation. This migration
-- deliberately creates no public tables, no checkout/order/payment path, and no
-- inventory or supplier mutation. Browser callers receive only bounded JSON
-- through this RPC; commerce_private remains unexposed and has no table grants.
...
begin
  ...
  -- Account discovery intentionally returns only accounts for the current actor.
    ...
    select coalesce(jsonb_agg(row_data order by (row_data ->> 'name'), (row_data ->> 'sku')), '[]'::jsonb)
      ...
      from (
        select jsonb_build_object(
          ...
          'availableStock', coalesce(i.quantity_on_hand - i.quantity_reserved, 0)
        ) as row_data
  ...
  if v_action = 'reorder_draft' then
    ...
    return jsonb_build_object('orderId', v_order_id, 'lines', v_result,
      'notice', 'Current sellability and stock are shown. Reorder does not recreate an order or guarantee historical price or availability.');
  end if;
...
comment on function public.b2b_portal_v1(text, uuid, jsonb) is
  'CM-B2B-PORTAL-1A account-scoped boundary. Validates auth.uid plus active account membership before every scoped action. Saved-list writes only; no order, payment, inventory, supplier or automatic checkout mutation.';
